 ...
 *
 * 更新日志:
 * 1. 新增 🇱🇺 落地-SOCKS5 组，用于筛选所有 SOCKS5 协议的节点。
 * 2. 新增 🇹🇷 中转节点 组，用于排除 SOCKS5 节点外的其他所有节点。
 * 3. 新增 🔗 链式中转 组 (relay类型)，将中转组和落地组串联起来。
 * 4. 将 '🔗 链式中转' 添加到 '✈️ ‍起飞' 主选择组中。
 * 5. 修正了原始脚本的结构错误，将所有逻辑都包含在 main 函数内。
 * 6. 将 DNS 配置正确合并到主配置对象中。
 * 7. 清理了配置中的冗余项并增加了注释。
 */
function main(config) {
  ...
  const customProxyGroups = [
    ...
    },
    {
      ...
      timeout: 2000,
      'include-all': true,
      // !! 重要提示 !!: 排除所有名称包含 "SOCKS5" 的节点，以避免形成回环。
      // 如果您的 SOCKS5 节点命名不同，请在此处同步修改。
      'exclude-filter': 'Enzu',
    },
    {
      name: '🇱🇺 落地-SOCKS5',
      ...
      timeout: 2000,
      'include-all': true,
      // !! 重要提示 !!: 此规则依赖于您的 SOCKS5 节点名称中包含 "SOCKS5" 关键字。
      // 如果您的节点命名不同 (例如 's5', 'sock'), 请相应修改此 'filter' 值。
      filter: 'Enzu',
    },
    ...
    // --- 原始的策略组 ---
    {
      ...
      type: 'select',
      // 在最前面加入链式代理选项
      proxies: ['🔗 链式中转', '⚡ ‍低延迟', '🔧 ‍自建', '🕹️ EDT', '👆🏻 指定', '📜 天书'],
    },
    {
      name: '⚡ ‍低延迟',
      type: 'url-test',
